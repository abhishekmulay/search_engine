...
api.defaultIndex = 'elastic4_final'
...
api.queryBody = function(query, from, size) {
  return {
    ...
    "stored_fields": [
      "title", "url", "headers.last-modified", "headers.date", "depth", "author"
    ],
    "_source": false
  }
}
...
function cleanHitItem(item) {
    ...
    var author = item.fields.author && item.fields.author.length ? item.fields.author.join(", ") : "";
...
api.search = opt => {
  ...
  if (typeof query == 'string') {
    query = {
      multi_match: {
        query: query,
        fields: ['title^3', 'url', 'text', 'author']
      }
    }
  }
